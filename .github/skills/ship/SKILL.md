---
name: ship
description: 'Finish a change in this karabiner config repo: regenerate README.md from karabiner.json, commit, rebase on master, squash, fast-forward merge into master, and push. Use when done with a key binding change and want it on master without opening a PR.'
---

# Ship (finish feature → merge to master)

Solo-developer workflow for this Karabiner config repo. Take the current branch (possibly in a worktree), regenerate the docs, land it on `master` as a single commit via fast-forward merge, and push to `origin`. No PR.
...
Skip this step and steps 4–5 if you are already on `master` in the main checkout; go straight to step 6.
...
- The main worktree is `~/.config/karabiner` — the live config Karabiner-Elements reads. After merging a `karabiner.json` change there, the new bindings take effect immediately; no reload is needed. Verify the key actually works before considering the change done.
