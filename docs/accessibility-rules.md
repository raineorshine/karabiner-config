...
"$HOME/.config/karabiner/scripts/bin/ax-press.sock"` — while a shell runs the binary itself with the
same arguments, which is what `--dump` and `--dry-run` are for. Some rules perform AXShowMenu rather
than AXPress — see "Context menus".

The helper itself — the resident server, how it gets Chromium to expose a tree, its Accessibility
grant, rebuilding it, and what a press costs — is [ax-press-helper.md](ax-press-helper.md). This file
is about writing a rule against it.
...
handlers sit there too, which is what explains a press that does nothing (see "When a press logs success"). Search it
...
the dumps for what appeared. An empty query matches nothing (see "Finding the label"): a first run of
...
press refusal outside Karabiner (the confused-deputy note in [ax-press-helper.md](ax-press-helper.md)) means a screen cannot be reached from
...
rule mechanism: a Karabiner-spawned `osascript`'s permissions are the unpredictable case "The Accessibility grant" in
[ax-press-helper.md](ax-press-helper.md) describes.
...
## When a press logs success and nothing happens
...
## Picking one control out of several
...
## Sequencing, and sharing a chord with the app
...
input, so the key was swallowed outright. A typing key wants the shape in the next paragraph instead.
...
## What the apps look like to accessibility
